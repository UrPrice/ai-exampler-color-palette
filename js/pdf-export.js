...
/**
 * Экспортирует текущий лист персонажа в PNG-изображение.
 * Использует html2canvas, который уже включён в html2pdf.bundle.min.js.
 */
function exportImage() {
  ...
  /* ——— 3. Восстановление оригинальных элементов ——— */
  function restore() {
    swaps.forEach(({ original, replacement }) => {
      if (replacement.parentNode) replacement.parentNode.replaceChild(original, replacement);
    });
  ...
  /* ——— 4. Рендер через html2canvas и скачивание PNG ——— */
  html2canvas(element, {
    scale: 2,
    useCORS: true,
    backgroundColor: '#c7bcae',
    scrollX: 0,
    ...
    restore();
    toast('Ошибка при создании изображения');
  });
}
